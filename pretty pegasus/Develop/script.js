...
function passwordCriteria() {
  // passlength will be read as number 
  var passLength = Number(prompt('How many characters should your password contain?'));

  // Ensure value must be entered as number
  if (Number.isNaN(passLength)) {
    alert('Desired length must be entered as a number');
    return null;
  }

  // If user does not input number, they will view alert and function will start over
  if (!passLength) {
  ...
  // If user presses cancel on special inquiry
  if (!includeSpecials) {
    alert('Password will not contain special characters');
  }

  var includeUppers = confirm("Click 'OK' to inlcude uppercase letters in your password");

  // If user presses cancel on uppercase inquiry
  if (!includeUppers) {
  ...
  // If user selects cancel on lowercase inquiry
  if (!includeLowers) {
  ...
  // If user selects cancel on numbers inquiry
  if (!includeNumbers) {
  ...
  // Users selections stored into object 
...
// Function to generate a random password based on the user's selected criteria 
function generatePassword() {
  ...
  //repeating if statements to join arrays of characters that user selects to the allPossible array
  ...
  // will return 'result' array without commas or spaces
  return result.join('');
}
